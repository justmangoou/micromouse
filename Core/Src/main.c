...
volatile uint8_t vl53l0x_ready_flags = 0;
VL53L0XData l_dist_data, r_dist_data, f_dist_data;
volatile uint8_t dist[3];
float yaw;
...
static void UpdateYaw(float dts, uint16_t encoder_cnt);
...
int main(void)
{
  ...
  /* USER CODE BEGIN WHILE */
    ...
    const float dt_s = (float)dt / 1000.0f;
...
static void MX_GPIO_Init(void)
{
  ...
  /*Configure GPIO pin : VL53L0X_R_XSHUT_Pin */
  GPIO_InitStruct.Pin = VL53L0X_R_XSHUT_Pin;
  GPIO_InitStruct.Mode = GPIO_MODE_OUTPUT_PP;
  GPIO_InitStruct.Pull = GPIO_NOPULL;
  GPIO_InitStruct.Speed = GPIO_SPEED_FREQ_LOW;
  HAL_GPIO_Init(VL53L0X_R_XSHUT_GPIO_Port, &GPIO_InitStruct);
...
static void VL53L0X_Config(const uint8_t addr)
{
...
static void VL53L0X_InitAll(void)
{
    ...
    HAL_Delay(10);
    VL53L0X_Config(VL53L0X_L_ADDR);

    HAL_GPIO_WritePin(VL53L0X_R_XSHUT_GPIO_Port, VL53L0X_R_XSHUT_Pin, GPIO_PIN_SET);
    HAL_Delay(10);
    VL53L0X_Config(VL53L0X_R_ADDR);

    HAL_GPIO_WritePin(VL53L0X_F_XSHUT_GPIO_Port, VL53L0X_F_XSHUT_Pin, GPIO_PIN_SET);
    HAL_Delay(10);
    VL53L0X_Config(VL53L0X_F_ADDR);
}

static void UpdateYaw(const float dts, const uint16_t encoder_cnt) {
    float gyro_z = MPU6050_GetZAngle();
    yaw += MPU6050_GetZAngle() * dts;

    static uint16_t prev_encoder_cnt;
    int16_t delta_encoder_cnt = encoder_cnt - prev_encoder_cnt;
...
static void Button_Handler(void)
{
    ...
    if (button_pressed) {
        ...
        if (button_pressed_duration < BUTTON_LONG_PRESS_TIME) {
...
void HAL_GPIO_EXTI_Callback(const uint16_t GPIO_Pin) {
    if (GPIO_Pin == VL53L0X_L_GPIO1_Pin) vl53l0x_ready_flags |= 0x01;
    if (GPIO_Pin == VL53L0X_R_GPIO1_Pin) vl53l0x_ready_flags |= 0x02;
    if (GPIO_Pin == VL53L0X_F_GPIO1_Pin) vl53l0x_ready_flags |= 0x04;
}
